...
(function(exports, window) {
	...
	var Game = Utils.extendTo(GameObject, function(timer) {
		...
		this.on('tick', function(delta) {
			...
			if (self.accelerationHistory.length > 0) {
				...
				self.accelerationX = self.accelerationXPre;
				self.accelerationY = self.accelerationYPre;
				self.accelerationZ = self.accelerationZPre;
			}
	...
	Game.prototype._tick = function(delta, callback) {
		...
		if (self._fixedStep) {
			if (Math.abs(delta - self._fixedStep) < 2) {
				delta = self._fixedStep;
			}

			self._remaining += delta;

			//console.log(self._fixedStep-self._remaining);
			...
			while (self._remaining >= self._fixedStep) {
				...
				// available time
				self.trigger('tick', self._fixedStep / 1000, self._timestamp);
